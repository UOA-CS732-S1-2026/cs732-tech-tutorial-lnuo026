...
### Computed Values
- `tileMap` (useMemo) — Randomly generates the grass tile grid once and caches it, prevents grass from re-randomizing on every render .
- `pixelFont` — Applies pixel font style only after `useFonts()` finishes loading, otherwise returns `{}` 
